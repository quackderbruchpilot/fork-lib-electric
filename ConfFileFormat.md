...
The diagram displays `2PB1 (Battery 1)` and `3PP (DC BAT BUS)`. The
...
existing parser; its normal comment rules also apply. Underscores and
punctuation in the label are displayed literally. Only one nonempty
`GUI_LABEL` is allowed per component. It is available for all electrical
component types, including buses, but not for a free-standing `LABEL_BOX`.
For `LOADCB`, the label belongs to the load, not its autogenerated breaker
or bus; declare a separate `CB` to give the breaker its own BMK and label.

Labels appear in component symbols, detail views, bus breaker lists and
the detail view's source description. They do not override `GUI_INVIS`,
`GUI_VIRT` or the requirement for `GUI_POS`. Without a label, the existing
name presentation is unchanged. Labeled components show the exact BMK,
